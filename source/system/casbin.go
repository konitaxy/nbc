...
func (c *casbin) CheckDataExist() bool {
	if errors.Is(global.GVA_DB.Where(adapter.CasbinRule{PType: "p", V0: "888", V1: "/user/getUserInfo", V2: "GET"}).First(&adapter.CasbinRule{}).Error, gorm.ErrRecordNotFound) { // 判断是否存在数据
		return false
	}
	return true
}
