...
<div class="search-bar">
  ...
  <div class="share-balance-row">
    <span class="share-balance-label">{{ $t('lang.share_card_balance') }}：</span>
    <span v-if="shareBalanceLoading" class="share-balance-value">...</span>
    <span v-else class="share-balance-value">{{ shareBalanceText }}</span>
    <el-button
      class="share-balance-btn"
      type="primary"
      icon="refresh"
      :loading="shareBalanceLoading"
      @click="fetchShareBalance">
    </el-button>
    <el-button
      v-if="$userStore.hasRole(5)"
      class="share-balance-btn"
      type="primary"
      @click="openShareRechargeDialog">
      {{ $t('lang.recharge') }}
    </el-button>
    <el-button
      v-if="$userStore.hasRole(5)"
...
<el-table ref="cardListTableRef" :data="tableData" style="width: 100%" border show-overflow-tooltip row-key="ID" @selection-change="onCancelSelectionChange">
  ...
  <!-- 表格列 - 卡片余额（共享主卡显示实际金额） -->
  <el-table-column :label="$t('lang.card_balance')" width="120">
    <template #default="{ row }">
      {{ formatTableCardBalanceDisplay(row) }}
    </template>
  </el-table-column>
  <!-- 表格列 - 卡币种 -->
  <el-table-column prop="currency" :label="$t('lang.card_currency')" width="100"></el-table-column>
  <!-- 表格列 - 卡类型 -->
  <el-table-column prop="cardLevel" :label="$t('lang.card_type')" width="170">
    <template #default="{ row }">
      <template v-if="row.cardModel === 'SHARE'">
        <span>{{ $t('lang.share_card') }}</span>
        <el-tag v-if="row.cardLevel === 'MasterCard'" type="success" size="small" style="margin-left: 8px;">{{ $t('lang.master_card') }}</el-tag>
        <el-tag v-if="row.cardLevel === 'SubCard'" type="primary" size="small" style="margin-left: 8px;">{{ $t('lang.sub_card') }}</el-tag>
      </template>
      <span v-else>{{ getCardTypeLabel(row.cardLevel) }}</span>
    </template>
  </el-table-column>
  <!-- 表格列 - 创建时间 -->
  ...
  <el-table-column :label="$t('lang.actions')"  :width="isMobile ? 110 : (locale === 'zh' ? 270 : 350)"  fixed="right" :show-overflow-tooltip="false">
    <template #default="scope">
      <el-dropdown class="d-block d-sm-none">
        ...
        <template #dropdown>
          <el-dropdown-menu>
            ...
            <el-dropdown-item v-if="$userStore.hasRole(5)" @click="handleAdjustLimit(scope.row)">{{ $t('lang.adjust_limit') }}</el-dropdown-item>
      ...
      <div class="action-buttons d-none d-sm-flex display-flex"> 
        ...
        <el-button v-if="$userStore.hasRole(5)" type="primary" size="small" @click="handleAdjustLimit(scope.row)">{{ $t('lang.adjust_limit') }}</el-button>
      ...
      align-center>
        <el-form :model="activeCardForm" label-position="top" class="open-card-form">
            ...
            <div v-if="cardForm.card" class="open-card-fields">
              ...
              <el-form-item :label="$t('lang.card_holder')">
  ...
  >
    <el-form label-position="top" class="p-4">
      ...
      <el-form-item :label="$t('lang.available_balance')">
        <el-input :model-value="formatCardBalanceDisplay(currCard)" disabled>
          <template v-if="currCard && !isNoLimitBalanceByTotalAuth(currCard)" #append>{{ currCard.currency }}</template>
        </el-input>
      </el-form-item>
  ...
  <!-- 共享余额充值（gzy matrix transfer_in + 扣系统钱包） -->
  <el-dialog
    v-model="dialogs.shareRechargeDialogVisible"
    :title="$t('lang.share_card_balance') + ' - ' + $t('lang.recharge')"
    width="400px"
    align-center
    destroy-on-close>
    <el-form label-width="auto">
      <el-form-item :label="$t('lang.wallet_balance')">
        <span>{{ userStore.userInfo.wallet == null ? '0' : userStore.userInfo.wallet.balance }} USD</span>
      </el-form-item>
      <el-form-item :label="$t('lang.recharge_amount')" required>
        <el-input-number
          class="w-100"
          v-model="shareRechargeForm.transferAmount"
          :min="0.01"
          :precision="2"
          :controls="false"
          :placeholder="$t('lang.recharge_amount')">
        </el-input-number>
      </el-form-item>
    </el-form>
    <template #footer>
      <el-button @click="dialogs.shareRechargeDialogVisible = false">{{ $t('lang.cancel') }}</el-button>
      <el-button type="primary" :loading="loading.shareRechargeLoading" @click="handleShareRechargeConfirm">
        {{ $t('lang.confirm') }}
      </el-button>
    </template>
  </el-dialog>

  <!-- 共享余额提现（gzy matrix transfer_out + 入账系统钱包） -->
  <el-dialog
    v-model="dialogs.shareWithdrawDialogVisible"
    :title="$t('lang.share_card_balance') + ' - ' + $t('lang.withdraw')"
    width="400px"
    align-center
    destroy-on-close>
    <el-form label-width="auto">
      <el-form-item :label="$t('lang.share_card_balance')">
        <span>{{ shareBalanceText }}</span>
      </el-form-item>
      <el-form-item :label="$t('lang.withdrawal_amount')" required>
        <el-input-number
          class="w-100"
          v-model="shareWithdrawForm.transferAmount"
          :min="0.01"
          :precision="2"
          :controls="false"
          :placeholder="$t('lang.withdrawal_amount')">
        </el-input-number>
      </el-form-item>
    </el-form>
    <template #footer>
      <el-button @click="dialogs.shareWithdrawDialogVisible = false">{{ $t('lang.cancel') }}</el-button>
      <el-button type="primary" :loading="loading.shareWithdrawLoading" @click="handleShareWithdrawConfirm">
        {{ $t('lang.confirm') }}
      </el-button>
    </template>
  </el-dialog>
  ...
  import { setCardGroup,listCardGroup,listCardHolder,remarkCard,syncCard,addCardHolder,fetchCardHolderAddress,listCardBin,createCard,frozenCard,listCard,rechargeCard,withdrawCard,adjustSubCardLimit,getGzyShareBalance,gzyShareRecharge,gzyShareWithdraw } from '@/api/finance';
  ...
  /** totalAuthLimit 为 0 时余额展示「不限额」（兼容 total_auth_limit） */
  const rawTotalAuthLimit = (row) => row?.totalAuthLimit ?? row?.total_auth_limit
  const isNoLimitBalanceByTotalAuth = (row) => {
    const raw = rawTotalAuthLimit(row)
    if (raw == null || raw === '') return false
    return Number(raw) === 0
  }
  const formatCardBalanceDisplay = (row) => {
    if (!row) return ''
    return isNoLimitBalanceByTotalAuth(row) ? t('lang.no_limit_balance') : row.balance
  }
  /** 表格：共享卡主卡始终展示实际余额，不使用「不限额」替代 */
  const formatTableCardBalanceDisplay = (row) => {
    if (!row) return ''
    if (row.cardModel === 'SHARE' && row.cardLevel === 'MasterCard') {
      return row.balance == null || row.balance === '' ? '' : row.balance
    }
    return formatCardBalanceDisplay(row)
  }
  const isUnlimitedAuthLimitRow = (c) => {
    if (!c) return false
    if (c.authLimitFlag === 'N') return true
    return isNoLimitBalanceByTotalAuth(c)
  }
  const formatCurrentAuthLimitDisplay = (c) => {
    if (!c) return ''
    if (isUnlimitedAuthLimitRow(c)) return t('lang.no_limit_balance')
    return c.totalAuthLimit
  }
  const showCurrentAuthLimitCurrencyAppend = (c) => c && !isUnlimitedAuthLimitRow(c)
  ...
  const dialogs =reactive({
    ...
    shareRechargeDialogVisible:false,
    shareWithdrawDialogVisible:false,
  })
  ...
  const loading = reactive({
    ...
    shareRechargeLoading:false,
    shareWithdrawLoading:false,
  })
  ...
  const shareBalance = ref(null)
  const shareBalanceLoading = ref(false)
  const shareBalanceText = computed(() => {
    if (!shareBalance.value) return '--'
    const amount = shareBalance.value.realTimeBalance ?? '--'
    const currency = shareBalance.value.currency || 'USD'
    return `${amount} ${currency}`
  })
  const fetchShareBalance = () => {
    shareBalanceLoading.value = true
    getGzyShareBalance({ currency: 'USD' }).then(res => {
      if (res.code === 0) {
        shareBalance.value = res.data || null
      }
    }).finally(() => {
      shareBalanceLoading.value = false
    })
  }
  const shareRechargeForm = reactive({
    transferAmount: null,
  })
  const shareWithdrawForm = reactive({
    transferAmount: null,
  })
  const openShareRechargeDialog = () => {
    shareRechargeForm.transferAmount = null
    dialogs.shareRechargeDialogVisible = true
  }
  const openShareWithdrawDialog = () => {
    shareWithdrawForm.transferAmount = null
    dialogs.shareWithdrawDialogVisible = true
  }
  const handleShareRechargeConfirm = () => {
    const amount = Number(shareRechargeForm.transferAmount)
    if (!amount || amount <= 0) {
      ElMessage.warning(t('lang.recharge_amount'))
      return
    }
    loading.shareRechargeLoading = true
    gzyShareRecharge({
      currency: 'USD',
      transferAmount: String(amount),
    }).then(res => {
      if (res.code === 0) {
        ElMessage.success(t('lang.success') || 'Success')
        dialogs.shareRechargeDialogVisible = false
        fetchShareBalance()
        userStore.GetUserInfo()
      }
    }).finally(() => {
      loading.shareRechargeLoading = false
    })
  }
  const handleShareWithdrawConfirm = () => {
    const amount = Number(shareWithdrawForm.transferAmount)
    if (!amount || amount <= 0) {
      ElMessage.warning(t('lang.withdrawal_amount'))
      return
    }
    loading.shareWithdrawLoading = true
    gzyShareWithdraw({
      currency: 'USD',
      transferAmount: String(amount),
    }).then(res => {
      if (res.code === 0) {
        ElMessage.success(t('lang.success') || 'Success')
        dialogs.shareWithdrawDialogVisible = false
        fetchShareBalance()
        userStore.GetUserInfo()
      }
    }).finally(() => {
      loading.shareWithdrawLoading = false
    })
  }
  onMounted(()=>{
    handleListCardBin()
  ...
  const handleListCardBin = () => {
    listCardBin({ ...holderSearch, cardModel: 'SHARE' }).then((res) => {
  ...
  const handleActiveCardConfirm = () => {
    ...
    // 共享卡固定开子卡：不传主卡，仅传授权额度；shareMode=1 后端子账号取主账号 matrixAccount
    if (cardForm.value.card && cardForm.value.card.cardModel === 'SHARE') {
  ...
  const handleActiveCard = () => {
    cardForm.value.totalAuthLimit = 1
...
const handleAdjustLimit = (row) => {
  const unl = isUnlimitedAuthLimitRow(row)
